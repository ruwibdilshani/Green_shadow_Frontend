...
$("#save-crop-popup button").click(function () {
  ...
  if (
    !validateCrop(cropName, cropScientificName, cropSeason, cropType, image)
  ) {
    return;
  }
...
function loadCropDataToUpdatePopup(cropCode) {
  getCrop(cropCode)
    .then((crop) => {
      ...
      $("#update-crop-popup .crop-scientific-text").val(
      ...
      const file = base64ToFile(
        base64ToImageURL(crop.cropImage),
        "crop-image.png"
      );
      console.log(file);
...
$("#update-crop-popup button").click(function () {
  ...
  const cropScientificName = $(
    "#update-crop-popup .crop-scientific-text"
  ).val();
